#!/usr/bin/env node
/**
 * Demo CLI for gawain-shopify-plugin
...
function parseCliArgs(): { productPath: string } {
  ...
  if (values.help) {
    console.info(`
gawain-shopify-plugin Demo CLI
...
async function main(): Promise<void> {
  console.info('=== Gawain Shopify Plugin Demo ===\n');
  ...
  if (!validateShopifyProduct(rawProduct)) {
    console.error('Invalid product format. Expected Shopify product structure.');
    process.exit(1);
  }
